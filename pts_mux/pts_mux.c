...
#define RESET		"\033[0m"
#define TV2MSEC(time)	(uint64_t)(time.tv_sec * 1000000 + time.tv_usec)
...
/*			gettimeofday(&systime, NULL);
			fprintf(debugFd, "%lu Client@%u is deleted from the polling list. numFds=%d\n", TV2MSEC(systime), pFd, numFds);
			fflush(debugFd);*/
			#endif
		}
		close(pFd);
	...
	#ifdef DEBUG
/*	gettimeofday(&systime, NULL);
	fprintf(debugFd, "%lu Master@%u is deleted from the polling list. numFds=%d\n", TV2MSEC(systime), masterFd, numFds);
	fflush(debugFd);*/
...
int main(int argc , char *argv[])
{
	...
	#ifdef DEBUG
	logFile = NULL;
	while ((option = getopt (argc, argv, "hb:l:g:")) != -1)
	#else
	while ((option = getopt (argc, argv, "hb:g:")) != -1)
	#endif
	{
		switch (option)
		{
		case 'h':
			...
			fprintf(stderr, "-b baudrate\t\tbaudrate to connect to a serial device (19200, 38400, 57600, 115200, 230400) [eg. -b 230400]\n");
			#ifdef DEBUG
			fprintf(stderr, "-l logFile\t\tSave stderr messages to a logFile [eg. -l /tmp/pts_mux.log]\n");
			#endif
	...
	if (getsockname(masterFd, (struct sockaddr *)&serverAddr, &len) == -1)
	{
	...
	if(add_poll_fd(masterFd) != -1)
	{
		#ifdef DEBUG
/*		gettimeofday(&systime, NULL);
		fprintf(debugFd, "%lu Master@%u is added to polling list. numFds=%d\n", TV2MSEC(systime), masterFd, numFds);
		fflush(debugFd);*/
		#endif
	}
	...
	do{
		...
		if ((ptsFd = create_pts(pts_array[pts_count], speed)) == -1)
			return 1;
		...
		if(add_poll_fd(ptsFd) != -1)
		{
			#ifdef DEBUG
/*			gettimeofday(&systime, NULL);
			fprintf(debugFd, "%lu pts@%s is added to polling list. fd=%d, numFds=%d\n", TV2MSEC(systime), pts_array[pts_count], ptsFd, numFds);
			fflush(debugFd);*/
			#endif
		}
	...
	sleep(1);
	printf("pts_mux has STARTED on port %d using PID %d", ntohs(serverAddr.sin_port), getpid());
	fflush(stdout);
...
//for (j = 0; j < size; j++)
//{
//	fprintf(debugFd, "%c", *(PTS_ptr->recvd_msg + PTS_ptr->recvd_len + j));
//}
//fprintf(debugFd, "\n");
...
//fprintf(debugFd, "%s\n", PTS_ptr->recvd_msg);
							...
							#ifdef DEBUG
							gettimeofday(&systime, NULL);
							fprintf(debugFd, "%lu pts@%s > client@%u message size=%d ", TV2MSEC(systime), PTS_ptr->ptsFile, PTS_ptr->clientFd, size);
							fflush(debugFd);
				...
				else
				{
					...
					if (size > 0)
					{
						...
						HASH_ITER(hh, hash_ptr, PTS_ptr, tmp_ptr)
						{
							if(strcmp(PTS_ptr->ptsFile, ptsFile) == 0)
							{
								...
								// Append newline character before sending the request
								...
								if(PTS_ptr->clientFd == -1)
								{
									...
									#ifdef DEBUG
									gettimeofday(&systime, NULL);
									fprintf(debugFd, "%lu client@%u > pts@%s message size=%d ", TV2MSEC(systime), clientFd, ptsFile, size);
									fflush(debugFd);
								...
								else
								{
									...
									#ifdef DEBUG
									gettimeofday(&systime, NULL);
									fprintf(debugFd, "%lu client@%u > msg_queue:pts@%s message size=%d ", TV2MSEC(systime), clientFd, ptsFile, size);
									fflush(debugFd);
									...
									// Print message list
									/*msg_queue_curr_ptr = msg_queue_start_ptr;
									fprintf(debugFd, "%lu msg_ptr -> ", TV2MSEC(systime));
					...
					else if (size == 0)
					{
						...
						if(del_poll_fd(clientFd) != 1)
						{
							#ifdef DEBUG
/*							gettimeofday(&systime, NULL);
							fprintf(debugFd, "%lu Client@%u is deleted from the polling list. numFds=%d\n", TV2MSEC(systime), clientFd, numFds);
							fflush(debugFd);*/
							#endif
						}
						...
						HASH_ITER(hh, hash_ptr, PTS_ptr, tmp_ptr)
						{
							if(PTS_ptr->clientFd == clientFd)
							{
								...
								#ifdef DEBUG
								gettimeofday(&systime, NULL);
								fprintf(debugFd, "%lu pts@%s > Client@%u request finished\n", TV2MSEC(systime), PTS_ptr->ptsFile, clientFd);
								fflush(debugFd);
								#endif
		...
		if(msg_queue_start_ptr != NULL)
		{
			...
			while(msg_queue_curr_ptr != NULL)
			{
				...
				if(PTS_ptr->clientFd == -1)
				{
					...
					#ifdef DEBUG
					gettimeofday(&systime, NULL);
					fprintf(debugFd, "%lu client@%u:msg_queue > pts@%s message size=%d ", TV2MSEC(systime), PTS_ptr->clientFd, PTS_ptr->ptsFile, size);
					fflush(debugFd);
		...
		if(pFds[0].revents & POLLIN)
		{
			...
			if(add_poll_fd(clientFd) != -1)
			{
				#ifdef DEBUG
/*				gettimeofday(&systime, NULL);
				fprintf(debugFd, "%lu Client@%u is added to the polling list. numFds=%d\n", TV2MSEC(systime), clientFd, numFds);
				fflush(debugFd);*/
				#endif
			}
		}
