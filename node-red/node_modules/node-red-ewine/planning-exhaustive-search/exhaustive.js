	...
	function exhaustive_search(n)
	{
		...
		this.on("input", function(msg)
		{
				...
				for(var p = 0; p < msg.PUT.length; p++)
				{
					...
					for(var i = 0; i < control_hdr.length; i++)
					{
						...
						else
						{
							...
							{
								msg.payload = "exhaustive-search: PUTs@uid=" + uid + " have different values";
								node.send([null, msg]);
								return;
							}
			...
			if(sample_Idx < node.sampleSet_data.length)
			{
				...
				{
					msg.payload = "exhaustive-search: empty parameter list";
					node.send([null, msg]);
					node.status({});
				}
			}
			...
			{
				msg.payload = "exhaustive-search: sample_Idx exceeded PUT limit";
				node.send([null, msg]);
				node.status({});
			}
		});
